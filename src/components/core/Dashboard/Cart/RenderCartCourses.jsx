...
import ReactStars from "react-rating-stars-component"
import { useDispatch, useSelector } from "react-redux"
import { removeFromCart, getCart } from "../../../../services/operations/authAPI"
import { useEffect } from "react"
import { selectCartItems } from "../../../../slices/cartSlice"
// import { useSelectorToken } from "../../../../utils/hooks" // assuming your token is stored in Redux or cookie

export default function RenderCartCourses() {
  const dispatch = useDispatch();
  const items = useSelector(selectCartItems) || [];

  const token = useSelector((state) => state.auth.token); // replace with actual token retrieval
  console.log("cart from redux: ", items)

  useEffect(() => {
    if (token) {
      dispatch(getCart(token));  // ✅ This will trigger Redux update
    }
  }, [dispatch, token]);
  ...
  const handleRemoveFromCart = async (courseId) => {
    try {
      await dispatch(removeFromCart(courseId));
      dispatch(getCart(token));  // ✅ Refresh after delete
    } catch (error) {
      console.error("Error removing course from cart:", error);
    }
  };
